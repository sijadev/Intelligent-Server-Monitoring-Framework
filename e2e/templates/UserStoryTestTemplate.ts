...
export class UserStoryTestTemplate {
  ...
  async activateTestProfile(): Promise<void> {
    ...
    console.log(
      `📊 Erwartete Daten: ${this.testProfile.expectedData.logEntries} Logs, ${this.testProfile.expectedData.problems} Problems`,
    );
    ...
    const profileExists = await this.page
  ...
  async validateExpectedVsActual(): Promise<{
    ...
    // Log Entries zählen
    const logStreamVisible = await this.dashboardPage.isElementVisible(
    ...
    const results = {
      logEntries: {
        ...
        match: this.isWithinTolerance(
          actualLogEntries,
          this.testProfile.expectedData.logEntries,
          0.1,
        ),
      },
      problems: {
    ...
    results.overallMatch =
      results.logEntries.match && results.problems.match && results.metrics.match;

    console.log(`📊 Soll/Ist Vergleich für ${this.testProfile.name}:`);
    console.log(
      `   Log Entries: ${results.logEntries.actual}/${results.logEntries.expected} ${results.logEntries.match ? '✅' : '❌'}`,
    );
    console.log(
      `   Problems: ${results.problems.actual}/${results.problems.expected} ${results.problems.match ? '✅' : '❌'}`,
    );
    console.log(
      `   Metrics: ${results.metrics.actual}/${results.metrics.expected} ${results.metrics.match ? '✅' : '❌'}`,
    );
