      ...
      [
        {
          ...
          action: async () => {
            ...
            console.log(
              `   Problems: ${crisis.problems.actual}/${crisis.problems.expected} ${crisis.problems.match ? '✅' : '🚨'}`,
            );
            console.log(
              `   Log Activity: ${crisis.logEntries.actual}/${crisis.logEntries.expected} ${crisis.logEntries.match ? '✅' : '🚨'}`,
            );
            console.log(
              `   System Metrics: ${crisis.metrics.actual}/${crisis.metrics.expected} ${crisis.metrics.match ? '✅' : '🚨'}`,
            );
...

        {
          ...
          action: async () => {
            ...
            for (const diagPage of diagnosticPages) {
              ...
              try {
                ...
                if (responseTime < 3000) {
                  ...
                  console.log(
                    `   ${diagPage}: ${responseTime}ms 🟡 ACCEPTABLE (für High Complexity)`,
                  );
                } else {
                  console.log(
                    `   ${diagPage}: ${responseTime}ms 🔴 SLOW (auch für High Complexity)`,
                  );
                }
              } catch (error) {
            ...
            const avgResponseTime =
              performanceResults.filter((r) => r.time > 0).reduce((sum, r) => sum + r.time, 0) /
              performanceResults.filter((r) => r.time > 0).length;

            console.log(`\n📊 PERFORMANCE ASSESSMENT (CI High Complexity Kontext):`);
            console.log(`   Durchschnitt Response Zeit: ${avgResponseTime.toFixed(0)}ms`);
            console.log(
              `   Bewertung: ${avgResponseTime < 5000 ? '🟢 System hält High Load aus' : '🔴 System überlastet'}`,
            );
          },
        },
          ...
          action: async () => {
            ...
            switch (crisisReport.recommendation) {
              ...
              case 'EMERGENCY_RESPONSE':
                ...
                console.log(
                  '📋 AKTIONEN: Emergency Response Team, Load Balancing prüfen, Service Degradation',
                );
                break;
            }
          },
      ...
      [
        {
          ...
          action: async () => {
            ...
            console.log(
              `   Container Problems: ${dockerCheck.problems.actual}/${dockerCheck.problems.expected} ${dockerCheck.problems.match ? '✅' : '⚠️'}`,
            );
            console.log(
              `   Deployment Activity: ${dockerCheck.logEntries.actual}/${dockerCheck.logEntries.expected} ${dockerCheck.logEntries.match ? '✅' : '⚠️'}`,
            );
          ...
          action: async () => {
            ...
            for (const service of containerServices) {
              ...
              try {
                ...
                console.log(
                  `   ${service.name}: ${serviceTime}ms (${serviceTime < 8000 ? 'Container Ready' : 'Container Stressed'})`,
                );
              } catch (error) {
            ...
            console.log(
              `🎯 Docker Profile Assessment: ${containerScore >= 75 ? '✅ Ready for Production' : '⚠️ Container Optimization Needed'}`,
            );
          },
        },
      ],
