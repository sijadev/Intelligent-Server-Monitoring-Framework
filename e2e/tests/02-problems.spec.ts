...
test.describe('MCP.Guard Problems Page', () => {
  ...
  test('should display problems list or empty state', async () => {
    ...
    if (problemsCount > 0) {
      ...
      const emptyStateVisible = await problemsPage.isElementVisible(
        '[data-testid="empty-state"], .empty-state, text=/no problems|empty/i',
      );
  ...
  test('should run comprehensive functionality test', async () => {
    ...
    console.log(
      `- Can resolve: ${results.canResolve ? '✅' : '⚠️ No problems or not implemented'}`,
    );
  });
