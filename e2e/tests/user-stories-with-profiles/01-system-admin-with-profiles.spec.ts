    ...
    await testTemplate.executeUserStoryTemplate(
      ...
      [
        ...
        },
        {
          ...
          context:
            'Sarah überprüft ob die Problemanzahl den Erwartungen des CI Medium Profils entspricht',
          action: async () => {
        ...
        },
        {
          ...
          action: async () => {
            const logVisible = await testTemplate.dashboardPage.isElementVisible(
            ...
            if (logVisible) {
              ...
              console.log(
                `📋 Sichtbare Log Einträge: ${logCount} (Erwartet für CI Medium: 4,830 total)`,
              );
        ...
        },
        {
          ...
          action: async () => {
            ...
            if (!comparison.overallMatch) {
              console.log(
                `ℹ️  Abweichung vom CI High Profil ist bei komplexen Szenarien erwartbar`,
              );
            } else {
      ...
      [
        {
          ...
          action: async () => {
            ...
            console.log(
              `   - Log Entries: ${comparison.logEntries.actual}/${comparison.logEntries.expected}`,
            );
            console.log(
              `   - Problems: ${comparison.problems.actual}/${comparison.problems.expected}`,
            );
        ...
        },
        {
          ...
          action: async () => {
            const pluginStatus = await page
              .textContent('[data-testid="plugin-status"]')
              .catch(() => '');

            // Suche nach TypeScript/JavaScript relevanten Plugins
            const hasJSRelatedPlugins =
              pluginStatus?.includes('typescript') ||
