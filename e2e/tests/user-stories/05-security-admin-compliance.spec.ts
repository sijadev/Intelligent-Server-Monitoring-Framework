...
test.describe('🔒 Security Administrator: Compliance & Monitoring', () => {
  ...
  test('As a Security Administrator, I need to audit system access and monitor for security issues', async ({
    page,
  }) => {
    ...
    await test.step('Verify secure system access patterns', async () => {
      ...
      const isSecureProtocol =
        currentUrl.startsWith('https://') || currentUrl.includes('localhost');
      ...
      const hasAuthElements = await page
        .locator('[data-testid="login"], [class*="auth"], [class*="login"], form[action*="auth"]')
        .isVisible()
        .catch(() => false);
      ...
      if (hasProblems) {
        ...
        for (let i = 0; i < Math.min(5, problemCount); i++) {
          ...
          const isSecurityRelated =
            problemText?.toLowerCase().includes('unauthorized') ||
        ...
        if (securityRelatedProblems > 0) {
          console.log(
            `⚠️  Found ${securityRelatedProblems} security-related problems requiring attention`,
          );
        } else {
  ...
  test('As a Security Administrator, I want to verify data handling and privacy compliance', async ({
    page,
  }) => {
    ...
    await test.step('Check for proper data handling indicators', async () => {
      ...
      const complianceIndicators = [
        ...
        {
          selector: '[data-testid="data-retention"], [class*="retention"]',
          type: 'Data retention',
        },
      ];
      ...
      console.log(
        `📊 Compliance features visible: ${complianceFeatures}/${complianceIndicators.length}`,
      );
    });
    ...
    await test.step('Verify activity logging capabilities', async () => {
      ...
      const logVisible = await dashboardPage.isElementVisible(
        '[data-testid="log-stream"], [class*="log"], [data-testid="activity"]',
      );
      ...
      if (logVisible) {
        ...
        const logContent = await page
          .textContent('[data-testid="log-stream"], [class*="log"], [data-testid="activity"]')
          .catch(() => '');

        // David looks for proper audit trail information
        const hasTimestamps =
          logContent?.includes(':') || logContent?.includes('AM') || logContent?.includes('PM');
        const hasUserInfo =
          logContent?.toLowerCase().includes('user') || logContent?.toLowerCase().includes('admin');
        const hasActionInfo =
          logContent?.toLowerCase().includes('access') ||
          logContent?.toLowerCase().includes('login');
    ...
    await test.step('Test data export and retention controls', async () => {
      ...
      if (!exportCapability) {
        console.log(
          'ℹ️  Data export functionality not visible - may need implementation for compliance',
        );
      }

      // Check for data retention policies or settings
      const retentionVisible = await page
        .locator('[data-testid="retention"], [class*="retention"], [data-testid="policy"]')
        .isVisible()
        .catch(() => false);
  ...
  test('As a Security Administrator, I need to monitor system vulnerabilities and access patterns', async ({
    page,
  }) => {
    ...
    await test.step('Assess system exposure and attack surface', async () => {
      ...
      const systemInfo = await page
        .textContent('[data-testid="system-info"], [class*="version"], [class*="build"]')
        .catch(() => '');

      // Check for version disclosure (potential security risk)
      const hasVersionInfo =
        systemInfo?.includes('version') ||
        systemInfo?.includes('v.') ||
        systemInfo?.includes('build');
    ...
    await test.step('Validate error handling does not leak sensitive information', async () => {
      ...
      try {
        ...
        const hasSensitiveInfo =
          errorContent?.includes('stack trace') ||
        ...
        const isCustomError =
          !errorContent?.toLowerCase().includes('nginx') &&
