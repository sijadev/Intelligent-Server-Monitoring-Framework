...
test.describe('👩‍💻 Developer: Code Issue Troubleshooting', () => {
  ...
  test('As a Developer, I want to quickly find code issues when my deployment fails', async ({ page }) => {
    ...
    await test.step('Look for code-specific error details', async () => {
      ...
      if (problemCount > 0) {
        ...
        for (let i = 0; i < Math.min(3, problemCount); i++) {
          ...
          const hasFilePath = problemText?.includes('.js') || problemText?.includes('.ts') || problemText?.includes('/');
  ...
  test('As a Developer, I need to understand the impact of my code changes', async ({ page }) => {
    ...
    await test.step('Check system logs for deployment-related entries', async () => {
      ...
      if (logsVisible) {
        ...
        const hasDeploymentLogs = logContent?.includes('deploy') || 
  ...
  test('As a Developer, I want to understand system patterns before making changes', async ({ page }) => {
    ...
    await test.step('Analyze current system load and patterns', async () => {
      ...
      const hasMetrics = await dashboardPage.isElementVisible('[data-testid="system-info"], [class*="metric"]');
      
      if (hasMetrics) {
        const metricsContent = await page.textContent('[data-testid="system-info"], [class*="metric"]');
    ...
    await test.step('Study historical problem patterns', async () => {
      ...
      if (hasProblems) {
        ...
        const allProblemsText = await page.textContent('[data-testid="problems-list"], table, .problems');
