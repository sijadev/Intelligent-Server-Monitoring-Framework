...
test.describe('👤 End User: System Experience', () => {
  ...
  test('As an End User, I want a simple and intuitive interface when checking system status', async ({ page }) => {
    ...
    await test.step('Navigate to IMF with minimal technical knowledge', async () => {
      ...
      const hasVisibleErrors = await page.locator('[data-testid="error"], .error-message, [class*="error"]').count();
    ...
    await test.step('Understand system status without technical jargon', async () => {
      ...
      for (const selector of statusIndicators) {
        const statusExists = await page.locator(selector).isVisible().catch(() => false);
        if (statusExists) {
          ...
          const isUserFriendly = statusText?.toLowerCase().includes('online') ||
    ...
    await test.step('Look for any alerts that affect end user work', async () => {
      ...
      if (problemCount > 0) {
        ...
        if (visibleProblems > 0) {
          ...
          const hasTechnicalJargon = problemText?.includes('undefined') ||
                                   problemText?.includes('null') ||
                                   problemText?.includes('Exception') ||
                                   problemText?.includes('Stack trace');
          
          if (hasTechnicalJargon) {
            console.log('⚠️  Technical jargon visible to end user - needs user-friendly translation');
          } else {
  ...
  test('As an End User, I need the system to be responsive and not frustrating to use', async ({ page }) => {
    ...
    await test.step('Test normal user navigation speed', async () => {
      ...
      const avgLoadTime = loadTimes.filter(t => t > 0)
                                  .reduce((sum, time) => sum + time, 0) / loadTimes.filter(t => t > 0).length;
      ...
      for (const selector of clickableElements) {
        try {
          ...
          if (exists) {
            ...
            if (responseTime < 2000) { // 2 seconds is reasonable for UI response
  ...
  test('As an End User, I want clear guidance when something goes wrong', async ({ page }) => {
    ...
    await test.step('Check for user-friendly error handling', async () => {
      ...
      const userErrorScenarios = [
        ...
        { action: 'Network timeout simulation', test: async () => {
      ...
      for (const scenario of userErrorScenarios) {
        ...
        try {
          ...
          const errorMessages = page.locator('[data-testid="error"], .error-message, [class*="error"]');
          ...
          if (errorCount > 0) {
            ...
            const isHelpful = errorText?.toLowerCase().includes('try again') ||
    ...
    await test.step('Verify contact/help information is accessible', async () => {
      ...
      if (!helpAvailable) {
        ...
        for (const location of commonHelpLocations) {
          ...
          if (helpText?.toLowerCase().includes('help') || 
    ...
    await test.step('Test graceful handling of system unavailability', async () => {
      ...
      const hasServiceStatusInfo = serviceStatus?.toLowerCase().includes('unavailable') ||
