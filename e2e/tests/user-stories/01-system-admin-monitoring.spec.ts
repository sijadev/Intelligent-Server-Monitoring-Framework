...
test.describe('👨‍💻 System Admin: Server Monitoring', () => {
  ...
  test('As a System Admin, I want to check system health when I arrive at work', async ({ page }) => {
    ...
    await test.step('Check system health indicators', async () => {
      ...
      const serverStatusVisible = await dashboardPage.isElementVisible('[data-testid="server-status"], [class*="status"], [class*="health"]');
      ...
      if (serverStatusVisible) {
        ...
        expect(['Online', 'Healthy', 'Running', 'Active'].some(s => 
    ...
    await test.step('Check recent activity logs', async () => {
      ...
      const recentLogsVisible = await dashboardPage.isElementVisible('[data-testid="log-stream"], [class*="log"], [class*="activity"]');
      ...
      if (recentLogsVisible) {
        ...
        const initialLogContent = await page.textContent('[data-testid="log-stream"], [class*="log"]').catch(() => '');
  ...
  test('As a System Admin, I need to investigate when problems are detected', async ({ page }) => {
    ...
    await test.step('Look for filtering options to manage workload', async () => {
      ...
      if (hasFilters) {
        ...
        await problemsPage.tryFilterBy('severity').catch(() => 
  ...
  test('As a System Admin, I want to monitor system resources during peak hours', async ({ page }) => {
    ...
    await test.step('Check resource utilization metrics', async () => {
      ...
      const metricsVisible = await dashboardPage.isElementVisible('[data-testid="system-info"], [class*="metric"], [class*="usage"]');
      
      if (metricsVisible) {
        console.log('📊 System metrics are visible');
        
        // Look for typical resource indicators
        const resourceMetrics = await page.textContent('[data-testid="system-info"], [class*="metric"], [class*="usage"]');
    ...
    await test.step('Verify system responsiveness during monitoring', async () => {
      ...
      await dashboardPage.refreshData().catch(() => console.log('ℹ️  Manual refresh not available'));
