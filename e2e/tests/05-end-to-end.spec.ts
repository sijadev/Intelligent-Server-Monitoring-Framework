...
test.describe('IMF End-to-End User Workflows', () => {
  ...
  test('responsive design workflow', async ({ page }) => {
    ...
    const hasMobileMenu = await dashboardPage.isElementVisible(
  ...
  test('accessibility workflow', async ({ page }) => {
    ...
    const hasSkipLinks = await dashboardPage.isElementVisible(
      '[href="#main"], [href="#content"], .skip-link',
    );
    const hasHeadings = await dashboardPage.isElementVisible('h1, h2, h3, h4, h5, h6');
    const hasLandmarks = await dashboardPage.isElementVisible(
      '[role="main"], [role="navigation"], main, nav',
    );
