...
export default function Logs() {
  ...
  const renderLogEntry = (log: LogEntry) => {
    ...
    return (
      <div
        key={`${log.timestamp}-${log.source}-${log.message.slice(0, 50)}`}
      ...
      >
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-900 break-words font-mono">{log.message}</p>
            {log.metadata &&
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      <Header title="System Logs" onRefresh={() => refetch()} isRefreshing={isLoading} />
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <div className="space-y-2">
                      ...
                      {filteredLogs.length > 50 && (
                        <div className="text-center py-4 text-gray-500">
                          Showing 50 of {filteredLogs.length} logs. Use filters to narrow down
                          results.
                        </div>
                      )}
                    </div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-500">
                    HTTP requests, WebSocket connections, and database operations
                  </p>
                </CardHeader>
                <CardContent>
                  {serverLogs.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">No server logs found.</div>
                  ) : (
                    <div className="space-y-2">
                      {serverLogs
                        .filter(
            ...
            <TabsContent value="plugins" className="mt-6">
              <Card>
                ...
                <CardContent>
                  {pluginLogs.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">No plugin logs found.</div>
                  ) : (
                    <div className="space-y-2">
                      {pluginLogs
                        .filter(
