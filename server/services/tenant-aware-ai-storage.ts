...
export class TenantAwareAIStorage {
  ...
  private validateAccess(progress: TenantAIProgress): boolean {
    ...
    const hasAccess = userPermissions.some((permission) => {
      return (
        permission === `${this.currentTenant.organizationId}:*` ||
  ...
  async shareModel(
    modelName: string,
    targetTenants: Partial<TenantContext>[],
    ...
    if (!result.success) {
      throw new Error(
        `Sharing failed: ${result.failed}/${result.shared + result.failed} targets failed`,
      );
    }
