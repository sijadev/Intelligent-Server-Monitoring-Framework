...
import { TenantAwareAIStorage, TenantContext, TenantAIProgress } from './tenant-aware-ai-storage.js';
...
export class EnhancedCrossTenantSharing extends TenantAwareAIStorage {
  ...
  async revokeSharing(
    ...
    const allProgress = await this.baseStorage.loadAIProgress() as TenantAIProgress[];
    const sharedModel = allProgress.find(p => 
  ...
  async getSharingStats(): Promise<{
    ...
    const modelsSharedByMe = allProgress.filter(p => 
      p.tenant_context?.shared_from && 
      p.sharing_metadata?.original_isolation_key === currentKey
    ).length;
    
    // Count models shared with me (models I can access but don't own)
    const modelsSharedWithMe = allProgress.filter(p => 
      p.isolation_key === currentKey && 
      p.model_name.startsWith('shared_')
    ).length;
    ...
    const mostSharedModel = Array.from(sharingCounts.entries())
