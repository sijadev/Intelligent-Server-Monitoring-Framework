...
const configSchema = z.object({
  ...
  SESSION_SECRET: z
    .string()
    .min(32)
    .default('your-secure-random-session-secret-replace-this-in-production'),
  ...
  PYTHON_API_URL: z
    .string()
    .optional()
    .transform((val) => (val && val !== '' ? val : 'http://localhost:8000'))
    .pipe(z.string().url()),
...
const parseConfig = () => {
  try {
    ...
    if (config.DEV_HTTPS_ENABLED) {
      ...
      console.log('  - TLS Cert:', config.DEV_TLS_CERT_PATH || '(not set)');
    }
...
export const isDockerEnvironment = () =>
  process.env.DOCKER === 'true' || process.env.DOCKER_CONTAINER === 'true';

// Configuration getters with fallbacks
export const getWorkspacePath = () =>
  config.IMF_TEST_WORKSPACE || config.WORKSPACE_PATH || config.TEST_MANAGER_WORKSPACE;
